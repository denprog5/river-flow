...
describe('Strings replacePrefix', function () {
    it('replaces prefix when present at the start', function () {
        expect(replacePrefix('foobar', 'foo', 'X'))->toBe('Xbar');
        expect(replacePrefix('hello', 'he', 'yo'))
            ->toBe('yollo');
    });

    it('returns original when prefix not at the start', function () {
        expect(replacePrefix('barfoo', 'foo', 'X'))->toBe('barfoo');
    });

    it('when empty prefix is provided, prepends replacement', function () {
