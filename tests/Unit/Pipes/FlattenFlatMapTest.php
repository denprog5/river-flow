...
describe('flatten and flatMap', function (): void {
    it('flattens arrays to specified depth and discards keys', function (): void {
        $input = [1, [2, [3]], 4];
    ...
    it('flattens iterables within iterables including Traversable', function (): void {
    ...
    it('flattens nested arrays with string keys and discards them', function (): void {
        $input = [ 'a' => 1, 'b' => ['x' => 2, 'y' => 3] ];
        expect(toArray(flatten($input, 1)))->toBe([1, 2, 3]);
    });

    it('flatMap maps and flattens by one level when callback returns iterables', function (): void {
        $input = [1, 2, 3];
        $out   = toArray(flatMap($input, fn (int $v): array => [$v, $v * 2]));
        expect($out)->toBe([1, 2, 2, 4, 3, 6]);
    });

    it('flatMap handles callbacks returning generators or scalars', function (): void {
        $input     = [1, 2];
        ...
        $out2 = toArray(flatMap($input, fn (int $v): int => $v * 3));
